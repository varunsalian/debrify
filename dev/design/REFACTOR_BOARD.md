...
## CI red on a9bd391c: one new failure introduced by #254; fix in flight
Fork CI on main fails `Run app tests against baseline allowlist` on every push since #254 (22baa526) with exactly one NEW failure: test/profiles/profile_source_guard_test.dart :: profile, download, deep-link, and remote logs stay redacted (allowlisted 12/12, none unused). Cause: #254 moved `_saveSeriesPosterToPlaylist` byte-for-byte from video_player_screen.dart (not scanned by the guard) into lib/services/playback/playlist_metadata_persistence.dart (scanned), carrying 10 raw `print(` calls; the service layer must log through the redacting `debugPrint` sink installed by `PrivacyLog.install()`. The Gate 11 tally below ("32 allowlisted / 0 new") was wrong: gate11-main/test-run.log line 16295 records this very failure. Also verified: the xmltv_legacy_guide_retention failure is allowlisted and pre-existing (fails on upstream-based code too), not refactor-related. Fix branch fix/playlist-metadata-redacted-sink: route the 10 calls through `debugPrint` (foundation import), shrink tool/analyze_baseline.json by the 10 now-fixed AVOID_PRINT entries (448 to 438; no local --record because of SDK drift). Local: dart analyze clean; guard 14/14; #254 origin pin 4/4 (under testWidgets, debugPrint is synchronous and the pin's Zone print hook still captures every line). Upstream PR waits on fork CI green for this fix and a green main.
...
## #252/#253 merged (V1-B0/B1 episode display); gate10 required
Merged at 0b6171b67d1d4f7b30eb5e8e1928172056e9fd9f (B-0 at 7efc5900, B-1 at 269bf150); independent verdict ADMIT WITH NOTES for both (C:/Users/hunth/source/review-253/VERDICT.md). B-0: value object 14 fields in origin first-read order, lazy _seriesPlaylist evaluated at call; B-1: origin diff empty after renames, six callers rewritten, zero forwarders, contract-test marker repointed past the dissolved title wrapper, pin 8/8, 31 suites pass, analyzer 24/24, layering 52, baseline rows 296/297 re-keyed (path/line/column only, same code/message/severity/metadata), no new identities. Host 10,334 to 9,763 (-571), net -571 on player, no new owners. Did we make a difference? Five widgets and the display projection exist once with a real-screen pin. More? Remaining builders need value objects; TV override PLAN 2.3 conditioned on exists (recommend post-merge regression). Player 9,763 / 9,500 (237 remaining). Production merges since gate9: 2 (243, 250, 251, 242 prior). Third merge (252/253) triggers gate10.

## #242 merged (T4 provider resolution and source fetchers)
Merged at a9e28bf675eab3434e8119c234e1a0265b12fc50, exact reviewed 53737c0c (rebased over #249; conflict resolved to keep only the two fetcher aliases). Independent full source acceptance ADMIT WITH NOTES (C:/Users/hunth/source/review-242/VERDICT.md): pin a837ce3b precedes move 9c088975 and passes 22/22 with owner files absent, moved bodies differ only by the two declared renames plus formatting, PlaybackMeta byte-identical, exactly two tear-offs remain (seriesFetcherFor, movieFetcherFor: recorded closeout debt), fallback order and precedence loop preserved, 275 tests pass, analyzer 10/10, layering 52, baseline identity sets identical, all three CI green, EngineRegistry-in-tests claim replaced by a verified counter-example. Service 4,483 to 4,146 (-337), owners 313/76/44, net +96. Did we make a difference? The service no longer owns provider resolution; the fetchers have a matrix pin. More? Delete the two aliases after re-pointing quick_play_rules and the source-search pin; the multi-provider precedence gap stays documented. Player 9,960 / 9,500 unchanged. Production merges since gate9: 1.

## Gate9 15d8cac6 PASS with notes
Gate9 on 15d8cac6e1c3b3daec01737ae77d4edabcb3b651 (after 243/250/251): generic 6,466 passed / 33 exact known / 3 skipped, zero new and zero unused; analyzer drift set byte-identical to gate8 and, versus the parent, only the one 'value' deprecation #243 consolidated (matching its baseline edit); layering 52/77 unchanged ids; Python 55/55; Windows release built in 93 s with the uncommitted MSVC-2026 workaround applied for the build only; Android/native not run. God files: Player 9,960 / Search 5,879 / Magic TV 3,322 / Settings 2,908 / storage_service 1,741. Evidence C:/Users/hunth/source/gate9-main/REPORT.md. Production merges since gate9: 0. Queue: #252 admitted (rebased), #253 blocker fixed (contract-test marker) and rebased, #254 and #242 under independent review, V1-E and V1-F in progress.

## #251 merged (V1-W menu track-apply); gate9 required
Merged at 8b74bdacfa8cad2fc4692271b9b9452cd06dd95c, exact reviewed 8aefd095 (rebase of 9d31ec43 onto main for CI's layering base; files unchanged). Independent verdict ADMIT WITH NOTES (C:/Users/hunth/source/review-251/VERDICT.md): pin 55e7a149 precedes move, six bodies empty-diff after undoing session.x and the public names, every await and token/mounted re-check in order, SubtitleTrackSession +1 member, zero forwarders, 51 tests pass, analyzer 24/24, all three CI green on the rebased head. Local union check before merge on main+#251: merge tree clean, W pin + A pin + presentation suite 12/12, layering 52. Player 10,084 to 9,960 (-124), controller 1,183 to 1,310, net +3. Player is now 9,960 / 9,500 (460 remaining). Did we make a difference? The menu apply operations sit beside the diagnostics/download/persist methods they call. More? Lane F (identity snapshot) stacked on this; token-race branch stays source-preserved. Third production merge since gate8 (243, 250, 251): gate9 on this tip required before the next production merge. In flight: #242 T4 (review), #252/#253 V1-B (review), V1-E, V1-Y, V1-F.

## #250 merged (V1-A player HUD layer)
Merged at d4490894492d4086e671ba3075badd19dec9402d, exact reviewed 913f81ec (rebase of d5422635 onto main for CI's layering base; files unchanged). Independent verdict ADMIT WITH NOTES (C:/Users/hunth/source/review-250/VERDICT.md): pin 2387261f precedes move, 5/5 with the destination files absent, origin ranges 8634-8848 / 7748-7790 / 7495 / 927-928 identical after declared renames, List<Widget> spread into the existing Stack (no nested Stack), gate flags written under setState, 43 player tests pass, analyzer 24/24, layering equal to main, all three CI green on the rebased head. Player 10,334 to 10,084 (-250), owners 278+56, net +84, zero forwarders. Did we make a difference? The HUD band and the Stremio next-loading overlay have real owners with a real-screen pin. More? IPTV reconnect and auto-sync pills stay source-preserved (need seams); banner fade-unmounts remain. Production merges since gate8: 2; #251 (-124) is next and triggers gate9.

## Gate8 37aa0020 PASS with notes; #243 merged (G4-5)
Gate8 on 37aa002087ef054e0861d32fdbd136fa5aedecee (after 241/248/249): generic 6,443 passed / 33 exact known / 3 skipped, zero new and zero unused; analyzer 448 with the same 18 NEW / 19 UNUSED local-SDK drift as gate7 and zero new versus parent; layering 52/77 (the #248 row removed, nothing added); Python 55/55; Windows release built in 199 s with the uncommitted MSVC-2026 workaround applied for the build only; Android/native not run. Evidence C:/Users/hunth/source/gate8-main/REPORT.md.
Merged #243 at f6a3c201de4787d47dee56e80572d454ea936542, exact reviewed 640a7c05; independent verdict ADMIT WITH NOTES (C:/Users/hunth/source/review-243/VERDICT.md): merge tree exactly the eleven files, pin precedes move and passes 8/8 with the six owner files absent, six bodies verbatim after parameterisation, shape floor 490 to 484 recounted with the test's own regexes (twelve token sites to six), radius-only guard lists the dropdown (1) and result card (2) beside the I2 entry, baseline delta exactly -2 host / +1 owner on main's blob (449 to 448), 177 tests pass with only the allowlisted tv_sidebar_nav entry, analyzer 72 to 71, layering unchanged, all three CI green. TorBox 7,015 to 6,655 and Real-Debrid 6,387 to 6,033 (-714), owners 582, net -132, zero forwarders. NOTES: the G4-5 quirk entries supplied on the PR are integrated in this commit (TorBox in-folder search unreachable, three builders not converged, _toggleSearch retained). Did we make a difference? Five chrome widgets and one predicate exist once. More? Delete-progress dialog, torrent results view and view-selector bar want a value object; Phase 3 hosts can adopt CloudFolderViewMode and CloudSelectionBar. Player 10,334 / 9,500 unchanged on main; #250 (-250) and #251 (-124) admitted and awaiting CI after rebase. Production merges since gate8: 1.

## #249 merged (T3-F alias retirement); gate8 required
Merged at 2fa52cc564cebc56ac0329d666ed95eafa8d1095, exact reviewed 5e8b0030 (one commit on cb8e196c, rebuilt from the recorded seven-file scope because the previous team's prepared patch is not on this machine). Independent verdict ADMIT WITH NOTES (C:/Users/hunth/source/review-249/VERDICT.md): merge tree exactly the seven files, 21 forwarders deleted (19 to PlaybackCandidateRanking, 2 to PlaybackSourceSearch), 31 internal references qualified, 152 test receivers re-pointed (the board's 156 included four group-label strings left unchanged), test text identical after receiver normalisation, the one function-value use passes the owner static, 253 tests pass across 12 files, analyzer 10/10 identical, layering unchanged, all three CI green. Service 4,548 to 4,483 (-65), no pin needed because nothing moves. More? T4 #242 must rebase over this (conflict in the alias region) and keeps its two fetcher aliases as recorded closeout debt. Player 10,334 / 9,500 unchanged on main; V1-A #250 (-250) awaits independent review. Third production merge since gate7 (241, 248, 249): gate8 on this tip required before the next production merge.

## #248 merged (R3 router dialogs)
Merged at a31241bb, exact reviewed d675423f; independent verdict ADMIT WITH NOTES (C:/Users/hunth/source/review-248/VERDICT.md): merge tree exactly the nine expected files, pins a0f6683d/e61a9a56 precede move c91a015e, moved bodies byte-identical after renames, missing-presenter policy verified at all three sites (consent checks the presenter before the latch; import refused before confirmation/restore; pairing early return), both author mutations reproduced, 73 focused tests pass, all three CI green. Router 3,889 to 3,883 (-6), owners 162 under lib/widgets/remote, main.dart +2, net +158, zero forwarders; layering 53 to 52 (services-to-widgets pairing-dialog import row removed). Did we make a difference? The router no longer imports widgets for its dialogs and the three dialogs are independently testable. More? Snackbar presenter and the two frozen profile dialogs behind the same seam would drop the remaining material.dart row; non-blocking notes: presenter doc names _MyAppState (actual _DebrifyAppState), pin file order dependency documented. Player 10,334 / 9,500 unchanged. Production merges since gate7: 2; next production merge (#243 or #249) is the third and triggers gate8.
...
HostTorrentPlaybackService5337→4548 (-789), newowners901, netproduction+112. Twenty-one tearoff forwarders64lines are T3-F debt (updatedfrom18/55), including warm/search origin entrypoints; T3-F must own all three new origin suites before repoint/removal and precedesT4. Did we make a difference? Ranking/search now have separate callable owners with actual-origin coverage. Is there more? UI/provider/facade debt remains; this does not reduce player lines.

Required integrated gate534bcb70 assigned Cicero NOW: nativefirstpair, fulltests/allowlist, analyzer/layering, Pythonhelpers, Windows/ARM64 builds, forwarderledger. No nextproductionmerge untilgatepasses. Then240→237→239 under normalfinalunion/review/CI, then nextgate. Player still10720,1220remaining; othergodcounts unchanged from245 report. No user smoke blocker; no newdevicepass inferred.
...
**R3 MOVE AUTHORIZED after pairing handoff:** fetched exacte61a9a56 on a0f6683d; pairing commit adds only472-line `test/router_pairing_fallback_origin_pin_test.dart`, no lib changes. This explicitly activates the previously granted main registration line PLUS its necessary import. Implement the three-operation RouterDialogs presenter and real widget implementation in the exact scope below; no repeated registration approval is needed. Both origin pin commits precede move; author/reviewer44PASS and four caught pairing mutations are reported evidence, independent final production gates remain. Pairing test now owned by R3; test-harness registration of the REAL presenter may adapt to new boot composition with original assertions/cases unchanged and disclosed separately, not fake returned outcomes.

Pass the navigator key (or live getter) and existing PairingGate to fallback, never an eagerly captured context. Preserve no-navigator return, panel registration counts and the existing process-wide pairing-dialog latch. The fifth mutation is masked by that latch, not a caught mutation; do not replace it with per-call state. UI registration must not create new side effects on build or admit commands without consent. Publish a stacked R3 PR on R2; merge still waits R2 and normal source/behavior/CI gates. Expected2→1 router rows remains measured goal; material debt retained.
...
**Baseline lock supersession:** actual QA PR246/refactor/qa-analyzer-cleanup f79e107d contains five OUT-OF-GRANT paths (bridge/download/Qwen tooling), not the authorized13 files, and baseline equals current main a9b86b6 unchanged. Scope correction posted on PR246; do not merge as approved QA. Baseline lock transferred NOW to retry for independently reviewed33 AVOID_PRINT plus1 UNUSED_FIELD path-only correspondences, no count growth. QA13 production work may continue; no QA baseline push until retry lock release. Existing unpublished QA baseline edits must be retained/reconciled, not discarded. Prior QA baseline-lock paragraph below is superseded by this decision.
...
- [x] Consolidate coverage, state readers/writers, lifetime/async dependencies and actual origin-test evidence in NOTES, with an exhaustive class/member/field/directive index and detailed evidence links.
- [x] Decide move/keep/delegated/backlog/predecessor for every cohesive region. Parent decisions override worker suggestions; no code move is automatically authorized by a candidate label.
- [x] Publish ordered next steps below and the uncovered target gap.

### Next work, in order

- [ ] **Navigation/shuffle first:** confirm the no-callback policy contract, then pin real screen behavior before any move. Preserve the shared Random, lazy reads, unknown sizes and actual40% threshold. Forecast145–160 host lines; no earned credit yet.
- [ ] **Playback identity/adoption prerequisite:** assign all active playlist/index/URL/cache/token/reset writers to explicit commands before moving source/load workflows. No repeat of the rejected59/72-member host proxy.
- [ ] **Display metadata projection:** after the live-read contract, assess title/subtitle/enhanced metadata as one responsibility;218 gross body lines are not a net estimate.
- [ ] **PikPak retry owner:** prove real-host retry/cancel/error behavior and review its expanded interface before movement. Forecast368–398 host lines, unmeasured until an exact body/binding proposal.
- [ ] **Subtitle owner convergence:** resolve cache/token ownership and pin menu-versus-legacy differences before moving application operations into the existing owner.
- [ ] **Rendering last:** only compose HUD/controls/overlays after their state ownership is clear; preserve native keys, focus, context and necessary lifecycle adapters.

**Planning budget:** navigation+retry forecasts total513–558 host lines, leaving837–882 still without an accepted design. No target reduction, percentage or unsupported hour estimate. Sleep-only, tiny video configuration, banner-only and random-tile-only cuts are not active work. Full details and per-region decisions: [REFACTOR_NOTES.md](REFACTOR_NOTES.md#complete-player-inventory-and-orchestrator-decisions--september-7).

PR #244: merged `f65486a3`, reviewed head `cc02d0e2`; all three CI jobs passed. Origin and post-move pins plus independent verification passed (two actual host cases each); supporting source-guard correction passed 12 tests. Player reduction 139, new owner 147, net production +8. Three existing analyzer-baseline entries follow the moved fields by path only; count 449 and all other entry fields stay unchanged. Full analyzer helper 430/449 with zero new issues; same-checker layering 53/77, zero added/removed IDs. Actual merge tree `b9fcd2ec68347ac8f81882337de2d0b3f125ef41` matches the reviewed union. Inventory remains fixed at pre-merge bde119f5; transition sites are completed, not future credit.
...
- [x] **Integrated gate f371decc — PASS.** Native firstpair, both first-attempt builds,6234PASS/12known/2skip, goldens21known/helpers0/no unexpected or unused; analyzer430/4490new,Python55,layer53 unchanged. Forwarders Player202 (+32net), others204/133/23/0.
- [ ] **Player remainder — whole-file inventory before further moves.** The 57-input composition, standalone menu, command-only, tiny renderer configuration and scheduling-only sleep candidates were rejected for insufficient ownership benefit. No such candidate remains active. All four workers now inventory the fixed file; the orchestrator will publish the complete decision map and prerequisites. V1-10 remains open; 1,395 merged-main lines remain above target.
...
### Reviewer intake wave — September 7, explicit user authorization

Author for all seven lanes: user's reviewer, not a worker. Exclusive ownership below applies serially to stacked parent/child lanes. Supporting test/manifest/CODEMAP files listed explicitly; no player, services/playback, services/cloud or frozen god-file edits are authorized by this intake. Merge order: **233 R1 → 235 D1 → 234 I1 → 238 T3 → integrated gate → 240 R2 → 237 D2 → 239 I2 → integrated gate**. Existing full f371 player gate continues separately. Review stacked children only after their parent merges and their base is main; verify retargeting rather than assuming it. Each merge keeps pin/move commits and updates the board once. No player line credit.

- **R1 / #233 — merged a2b66bb2.** Branch `refactor/r1-remote-chunk-transfer`, reviewed-head candidate `4134a277db0f512f872a1237e1701cc55504745f`. Exact owned files: `lib/services/remote_control/remote_chunk_transfer.dart`; `lib/services/remote_control/remote_command_router.dart`; `lib/services/remote_control/remote_stale_source_notice.dart`; `test/remote_chunk_transfer_origin_pin_test.dart`.
- **D1 / #235 — merged8dab9f71.** Branch `refactor/d1-merged-detail-widgets`, reviewed-head candidate `44dbca9acd2332ccb1ac547279887cc2d5c22dbe`. Exact owned files: `lib/screens/merged_series_detail_screen.dart`; `lib/widgets/detail/detail_action_buttons.dart`; `lib/widgets/detail/detail_focus_chrome.dart`; `lib/widgets/detail/detail_rail_cards.dart`; `lib/widgets/detail/detail_tracker_sheets.dart`; `test/merged_detail_widgets_origin_pin_test.dart`.
- **I1 / #234 — merged748364b0.** Branch `refactor/i1-iptv-stage-widgets`, reviewed-head candidate `0c65aece6806568c5a808219e2adefcbde33a6f0`. Exact owned files: `lib/widgets/iptv/iptv_results_view.dart`; `lib/widgets/iptv/stage/iptv_rail_info.dart`; `lib/widgets/iptv/stage/iptv_stage_chip.dart`; `lib/widgets/iptv/stage/iptv_stage_floor.dart`; `test/iptv_stage_widgets_origin_pin_test.dart`; `test/theme/shape_manifest_test.dart`.
- **T3 / #238 — blocked: six moved search/engine functions lack actual origin-path pins.** Branch `refactor/t3-playback-candidate-ranking`, reviewed-head candidate `2c481e96d4ac5a84479935b9c40bc9453288e35f`. Exact owned files: `CODEMAP.md`; `lib/services/torrent_playback/playback_candidate_ranking.dart`; `lib/services/torrent_playback/playback_source_search.dart`; `lib/services/torrent_playback_service.dart`; `test/playback_candidate_ranking_origin_pin_test.dart`.
- **R2 / #240 — parent233 merged; retargeted to main, review queued after first intake gate.** Branch `refactor/r2-remote-transfer-bookkeeping`, reviewed-head candidate `3cc0cb0403464687d0e1b9865311c3d68102e8e1`. Exact owned files: `lib/services/remote_control/remote_command_router.dart`; `lib/services/remote_control/remote_legacy_consent.dart`; `lib/services/remote_control/remote_transfer_bookkeeping.dart`; `test/remote_transfer_bookkeeping_origin_pin_test.dart`.
- **D2 / #237 — parent235 merged; retargeted main, review follows first intake gate.** Branch `refactor/d2-merged-detail-tracker-status`, reviewed-head candidate `fc8ef5eb2f78d8c90a4828e1306432e75c1b0aa0`. Exact owned files: `lib/screens/merged_detail/detail_tracker_controller.dart`; `lib/screens/merged_detail/detail_trailer_controller.dart`; `lib/screens/merged_series_detail_screen.dart`; `test/merged_detail_tracker_status_origin_pin_test.dart`.
- **I2 / #239 — parent234 merged; retargeted main, review queued.** Branch `refactor/i2-iptv-preview-stage`, reviewed-head candidate `c1684b0c43ae0114620bfce63463f2d7a3323191`. Exact owned files: `lib/widgets/iptv/iptv_results_view.dart`; `lib/widgets/iptv/stage/iptv_cockpit_stage.dart`; `lib/widgets/iptv/stage/iptv_preview_rail.dart`; `lib/widgets/iptv/stage/iptv_preview_stage.dart`; `lib/widgets/iptv/stage/iptv_stage_state.dart`; `test/iptv_preview_stage_origin_pin_test.dart`; `test/theme/shape_manifest_test.dart`.

**Decisions:**

- **I1 Decision1 / I2 Decision2:** accept I1's111-line Leaves shortfall as explicit debt cleared by **I3, Phase3 dead-code deletion**. I1's body describes208 retained dead-class lines, not111;111 is the target shortfall, so do not conflate these measures. I3 owns `lib/widgets/iptv/iptv_results_view.dart`, `lib/widgets/iptv/stage/iptv_preview_rail.dart` and dedicated reachability tests (exact new test path at assignment). After I2, verify all callers and remove unreachable focus-stage/hints/keycap and rail TV/non-touch branches plus obsolete callback plumbing. No layout revival/feature work. I3 is queued for Phase3, not the next immediate reviewer wave.
- **T3 Decision1:** queue **T3-F, test-facing facade removal**, after T3, before any overlapping T4 service edit. Owns `lib/services/torrent_playback_service.dart`, `test/quick_play_rules_test.dart`, `test/filter_ladder_test.dart`, `test/torrent_playback_service_strings_test.dart`, and `test/playback_candidate_ranking_origin_pin_test.dart`. Repoint callers to existing ranking/search owners and remove18 forwarders (48 declaration/annotation lines plus7 comment lines,55 total); preserve origin chronology and behavior. No ranking/provider changes. Optional direct private-curation tests need separate explicit scope.
- **D2 Decision3, gate(c):** listed mechanical rules are admissible transformations to REVIEW, not automatic equivalence. Require per-body inverse/map preserving every live read before/after await, supplied context lifetime, disposal/mounted guards, setState lifecycle checks/mutation ordering and notification count. Any altered behavior or double rebuild is a reject. Final ruling on actual bodies occurs only after D1 lands. The same-shape V1 precedent alone is insufficient. D2 Decision2 same-commit input value objects needs explicit assessment against §2.3; no silent waiver.
- **R1 Decision5 / R2 Decision1:** queue **R3 dialog layering** after R2. Owns `lib/services/remote_control/remote_command_router.dart`, new `lib/widgets/remote/router_busy_dialog.dart`, new `lib/widgets/remote/remote_legacy_consent_dialog.dart` and dedicated dialog origin tests (exact test names at assignment). Move BOTH busy and legacy consent dialogs; keep protocol/payload/consent policy unchanged and verify actual layering IDs decrease. No services-to-widgets growth accepted by declaration alone; verify final dependency direction.
- **T3 Decision4:** accept the existing exact CODEMAP line-figure/directory hunk as lane-owned documentation. No M0 revert required. Serialize integration with player CODEMAP changes; no broader documentation edits.
- **R1 Decision2:** channel-config payload handler remains with existing config ownership; no intake expansion. R1 Decision3 CODEMAP additions may use a separate documentation-only lock after review, not an orchestrator edit. D1 naming with `Detail` prefix accepted.
- **I1 Decision2:** shared resolution expression may remain in the reviewed rail-info file; no unsolicited neutral-file move. I2 live getters must retain actual re-read semantics; no snapshot conversion.
- **Next wave:** T4 and D3 may be prepared source-only, with exact file manifests required before assignment; T4 waits T3-F and D3 must respect D2 completion/dependency. R3 queued above. I3 explicitly waits Phase3. These are not permission for cross-lane writes or automatic merges.
...
**L1 accessible pin source review:** fetched `origin/refactor/l1-launcher-android-tv-payload`, exact d7d357c94201d215a5edf5d970b31aa1d7c2a76a. Two files: launcher seam net17 and439-line origin test. No runtime reproduced by parent yet. Required narrow seam correction: current `await debugAndroidTvLaunch?.call(...) ?? await realBridge(...)` adds an await of null before normal production bridge invocation. Branch on a locally captured nullable override first, then await only the selected call; preserve bridge argument evaluation/error/launch behavior. Existing launcher-side seam scope covers this correction, not a bridge edit.

The proposed single-call toWidget replacement is incomplete: actual current callers also exist in `test/iptv_channel_sheet_responsive_test.dart` and `test/video_player_launcher_external_url_test.dart`. Do not remove that method and silently break the tests/public source API. Keep data-only args with a launcher-side exported extension preserving ordinary `args.toWidget()` call syntax as a possible bounded alternative, but enumerate member tear-offs/dynamic/subtype callers before acceptance; extensions are not identical to instance members. No test rewrites to conceal compatibility drift. Reviewer must present exact caller strategy before expanded move.

Pin commit explicitly says series-start seeds are separated on real clock to avoid ties: iteration-order tie behavior is source-preserved, NOT demonstrated tie coverage from these five cases. Correct notes/PR coverage claims accordingly or add actual tie pin.

**L1 revised seven-file proposal — September 7:** accept acyclic layout for preparation: `lib/services/launcher/launch_args.dart`, `launcher_provider_dispatch.dart`, `resolved_stream_cache.dart`, `entry_url_resolver.dart`, `android_tv_playback_payload.dart`, `android_tv_playlist_resolver.dart`, `android_tv_playback_payload_builder.dart`. Two exact re-exports preserve existing launch-args/dispatch importers; no consumer-wide migration or new runtime facade. All seven are under the stated launcher directory. This supersedes the three-destination-only preparation reservation, not final move admission.

Choose the existing launcher-side terminal seam in principle; do NOT move it to AndroidTvPlayerBridge or edit that file. Changing seam location provides no demonstrated test benefit and expands scope. The seam remains a17-line production item in the pin, never described as a test-only commit. Exact d7d357c9 remains unavailable after latest origin fetch; reviewer must publish a fetchable fork ref or provide its actual checkout for source review.

Grant `test/cloud_launcher_bulk_add_strings_test.dart` only for exact moved-marker correspondence if necessary, in addition to the previously granted Android TV MDBList marker suite. Markers that survive stay unchanged. Every assertion retained; source markers are not behavioral origin pins.

Expanded moves of redirect/cache, provider dispatch, resume forwarder and launch args require per-body existing-origin test mapping before application; the five payload cases cannot automatically cover this extra scope. Preserve cache instance/retirement, redirect errors/headers/timeouts, constructor/copyWith/toWidget behavior and public signatures. Actual launch_args contains toWidget returning VideoPlayerScreen: disclose its screen import and any retired old edge. No claim of pure data-only args or zero added layering IDs without measured full dependency graph; no duplicate service-to-screen edge accepted. Check DAG beyond seven siblings as well as within them. Leaves1987/new2050/net63 remain estimates until actual diff; no new production movement granted until accessible seam/pin and expanded origin/edge evidence are reviewed.
...
- **Conflict check:** no current player worker or seven-lane reviewer intake owns either file. Historical G17d launcher-hook entry is historical completed work, not an active reservation. L1 must preserve those hooks. Parent owns BOARD/NOTES; CODEMAP and shared test changes need explicit separately serialized scope. Both reviewers must report exact pin path, tested origin SHA and results on handoff. If a pin requires production changes, stop and report the required seam rather than making it.
...
- **#234 I1 merged748364b0:** exact0c65 allCIgreen; origin5PASS/current95IPTVPASS plus shape95PASS/1exactknownsidebarfailure (not rawgreen), analyzers0/layer53unchanged. Actualtree36dc3796 verifies6acceptedpayloads/alloutsideunchanged. IPTV7569→7080 (-489), owners523/net+34;111targetshortfall accepted withI3Phase3clearing slice,208deadclasslines separatemeasure. Shape2registeredfiles/floor490 preserved. Did we make a difference? Reachable stagewidget ownership leaveshost with preservedbehavior. Is more needed? I2composition/I3deadcode remain. Intake3of4; gateafter238 as userordered. Player11034 unchanged;239retargetedmain.

- **#235 D1 merged8dab9f71:** exact44db allCIgreen, independentorigin14PASS/current52PASS, scoped1inheritedINFO/full4304490new/layer53unchanged. Actualtreea68f8ed0 verifies6acceptedblobs andalloutside main unchanged. Detail5288→3143 (-2145), newwidgets2218, net+73;22widgets/21newkeys/4imports corrected in PRbody, zero methodforwarders withgoldalias retained. Did we make a difference? Presentational units are independently owned/importable; relocation is not netlogic deletion. Is more needed? D2 tracker/trailer ownership review and preservedquirks remain. Intake2of4 before fullgate; player11034/remaining1534 unchanged. D2retargetedmain.

- **#233 R1 merged a2b66bb2:** exact4134 allCIgreen; independent origin11PASS/current78PASS/scoped0/full4304490new/layer53unchanged; actualmerge tree6b2d4d44 matches reviewed union. Router5016→4300 (-716), newowners825, net+109, five receiver callbacks plus one notice effect; two debug delegates/constant alias retained. Historical pre-move execution author-reported; origin reproduction and chronology verified. Did we make a difference? Chunk receive state/protocol leaves singleton for dedicated ownership. Is more needed? R2bookkeeping and R3dialogs remain. Intake1of4 before checkpoint; player11034 unchanged.
- **Fullf371 gate PASS:**6234PASS/12known/2skip, goldens21known/configured2retries,actualhelpers0/no unexpected/unused. Nativebc46/current FIRSTPAIRPASS; analyzer430/4490new/Python55/layer53→53. SameASTStorage204/Search133/Player202/Magic23/Settings0; Player+32net (+33/-1 identities). Physical1741/5879/11034/3322/2908. Windows firstPASS ZIP515d9d557b18816f37457a9ff1a5e9356ddb2f4d3898cb10a6ee0facc1cb22db; ARM64firstPASS e06caf27e2b0a4dc0f8943c35c594e7cc477199ead7fadddf153796c3daae92a. No newdeviceclaim, baseline changes or native/build retry.
...
D2 active review blocker: candidate MDBList presenter omits three sheet arguments present in actual origin; prior author-reported preserved-chrome claim is contradicted by source. Restore before acceptance; per-body capture review ongoing.

D2 corrected ecca2d8e: both source regressions restored; independent18focusedPASS. Exact two store-only16/5input carriers granted same-commit sequencing exception, not a necessity or blanketprecedent. FreshCI/intakegate remain. R2 correctedfe748: availability->latch->presenter restored,6focusedPASS; currentLeaves411/net+170, body correction requested.

T3 gate(c/h) correction:203passed suites do not execute searchSeriesPackSources/searchCuratedSources or their two curators/engine listing/fetch functions. Source-inverse proof does not close this gap. Parent requested narrower ranking-only scope with exactshortfall/followup, or genuinely pin-before-move originhistory for allsix. No exception/merge authorized; firstintakegate waits correctedT3.
