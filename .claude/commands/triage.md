...
repo root; the Discord tooling lives in `dev/tool/discord/`. Secrets are in the gitignored
`dev/tool/discord/.dev.vars` (bot token) and `gh` is already authenticated for issue creation.
...
- Never print secrets; the bot token stays in `dev/tool/discord/.dev.vars`.
