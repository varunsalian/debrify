...
// PlaybackProgressStore, keyed by the launch identifiers.
//
// Harness copied from player_presentation_controls_origin_test.dart. The
// terminal gains the external `open` transport from
// player_playlist_navigation_origin_test.dart because a playlist launch opens
// media; TVMaze is answered by a canned HttpOverrides (cloud_folder_sort pin).
      ...
      () async {
        try {
          await tester.runAsync(() async {
            ...
            await TrackingPrefs.setWatchProgressSource(
          ...
          final before = await seededItem(tester);
          expect(before.containsKey('imdbId'), isFalse);
          expect(before.containsKey('posterUrl'), isFalse);
