...
// Navigation baseline against the owner host; only native initialization and
// construction are substituted. Assertions observe real Controls/menu commands
// and media opens, without replacing policy or accessing private State.
...
class _TrackedClient extends http.BaseClient {
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    final endpoint =
        uri.host == 'api.tvmaze.com' && uri.path == '/shows/1' ||
...
void main() {
  ...
  void reportClients(String label) {
    debugPrintSynchronously(
      'NAV_CLIENTS ${jsonEncode({
        ...
        'clients': clients.map((client) => {'creationStack': client.creationStack, 'sent': client.sent, 'closes': client.closeCalls, 'bodyStarted': client.bodyStarted, 'bodyDone': client.bodyDone}).toList(),
      })}',
    );
  }
  ...
  tearDown(() async {
    if (!cleanupSafe) {
      ...
      debugPrintSynchronously(
        'NAV_UNSETTLED resources retained at $fixtureRoot',
      );
    ...
    try {
      ...
      cleanupSafe = false;
      debugPrintSynchronously(
        'NAV_RESOURCE_CLOSE_FAILURE $error\n$stack root=$fixtureRoot',
      );
      ...
      if (resourcesClosed) {
        ...
        VideoPlayerScreen.debugEnsureMediaKitInitialized = null;
  ...
  int countPrefix(String prefix) =>
      observed.where((s) => s.startsWith(prefix)).length;
  int restoredTracks() => countPrefix('SubAuto: restore done');
  int addonDone() => observed
      .where((s) => s == 'SubAuto: ABORT — no IMDB ID for addon subtitle fetch')
      .length;
  int movieStarts() =>
      countPrefix('MovieMetadata: Checking filename at index ');
  int movieDone() => observed
      .where(
        (s) =>
            s.startsWith('MovieMetadata: No year pattern found at index ') ||
            s.startsWith(
              'MovieMetadata: No match found in Cinemeta for index ',
            ),
      )
      .length;
  bool metadataTail(bool series) => series
      ? observed
                    .where((s) => s == 'TVMaze: Searching for "navigation"')
                    .length ==
                1 &&
            observed
                    .where(
            ...
            printed
                    .where(
                      (s) =>
                          s ==
                          '  ⚠️ No valid identifier found, skipping poster save',
                    )
                    .length ==
                1
  ...
  void noErrors() {
    ...
    expect(
      observed.where(
        (s) =>
            s.contains('FAILED with exception:') ||
  ...
  bool autoRole(_TrackedClient c) {
    final a = c.creationStack.indexOf(
      'new AutoSkipSegmentProvider (package:debrify/services/skip_segment_service.dart:',
    );
    final b = c.creationStack.indexOf(
      'SkipSegmentProviders.create (package:debrify/services/skip_segment_service.dart:',
    );

    final e = c.creationStack.indexOf(
      '_VideoPlayerScreenState._loadSkipSegmentSettings (package:debrify/screens/video_player_screen.dart:',
    );
    return a >= 0 && b > a && e > b;
  }
  ...
  Future<void> runCase(
    ...
    Future<void> loadTail() async {
      ...
      await reach(
        tester,
        () =>
            terminal.properties != null &&
            terminal.properties!.events
                    .where((e) => e.startsWith('open:'))
                    .length ==
                target &&
            countPrefix('SubAuto: _restoreTrackPreferences entered (token=') ==
                target,
      );
      ...
      expect(
        binding.clock.now().difference(before),
      ...
      if (target > 1) {
        expect(
          terminal.properties!.events.lastWhere((e) => e.startsWith('open:')),
          endsWith(':play=true'),
          reason: 'Navigation opens the selected media with autoplay enabled',
        );
      }
      noErrors();
    ...
    Future<void> complete() async {
      terminal.properties!.reportCompleted();
      await loadTail();
    }
    ...
    Future<void> menuAction(String label) async {
      if (find.byType(PlayerMenuPanel).evaluate().isEmpty) {
        ...
        await reach(
          tester,
          () =>
              find.byType(PlayerMenuPanel).evaluate().length == 1 &&
              find
                      .descendant(
      ...
      final panel = tester.widget<PlayerMenuPanel>(
      ...
      if (!turnsOff) {
        ...
        expect(
          tester
              .widget<PlayerMenuPanel>(find.byType(PlayerMenuPanel))
              .continuousShuffle,
          isFalse,
        );
      }
        ...
        () async {
          try {
            ...
            final policy = await tester.runAsync(() async {
              ...
              final addons = await StremioSubtitleService.instance
                  .getSubtitleAddons();
            ...
            await tester.pumpWidget(
              MaterialApp(
                ...
                home: VideoPlayerScreen(
                  videoUrl: entries[start].url,
                  title: entries[start].title,
                  playlist: entries,
                  startIndex: start,
            ...
            await exercise(step, menuAction, current, controls, complete);
            noErrors();
            bodyPassed = true;
          } catch (error, stack) {
            primary = error;
            ...
            try {
              ...
              expect(
                ...
                isTrue,
                reason:
                    'Do not release dependent resources after incomplete policy/producer evidence',
              );
              ...
              // Private startup preference loaders have no public join.
  ...
  List<PlaylistEntry> collection() => const [
    PlaylistEntry(
      url: 'https://navigation.invalid/new.mp4',
      title: 'New 2004',
      sizeBytes: 1000,
    ),
    PlaylistEntry(
      url: 'https://navigation.invalid/below.mp4',
      title: 'Below 1998',
      sizeBytes: 399,
    ),
    PlaylistEntry(
      url: 'https://navigation.invalid/boundary.mp4',
      title: 'Boundary 2001',
      sizeBytes: 400,
    ),
    PlaylistEntry(
      url: 'https://navigation.invalid/unknown.mp4',
      title: 'Unknown 2003',
    ),
    PlaylistEntry(
      url: 'https://navigation.invalid/old.mp4',
      title: 'Old 1999',
      sizeBytes: 500,
    ),
  ];
  ...
  for (final mode in [PlaylistViewMode.raw, PlaylistViewMode.sorted]) {
    testWidgets(
      'actual ${mode.name} Controls follows supplied order both ways',
      (tester) async {
        ...
        await runCase(
          ...
          start: 0,
          series: false,
          exercise: (step, menu, current, controls, complete) async {
            expect(controls().onPrevious, isNull);
            for (final i in [1, 2]) {
              await step(true);
              expect(current(), entries[i].url);
            }
  ...
  testWidgets(
    'actual collection admits 40-percent boundary and unknown size in year order',
    (tester) async {
      ...
      await runCase(
        ...
        series: false,
        exercise: (step, menu, current, controls, complete) async {
          for (final i in [2, 3, 0]) {
            await step(true);
            expect(current(), entries[i].url);
          }
          expect(controls().onNext, isNull);
          for (final i in [3, 2, 4]) {
            await step(false);
            expect(current(), entries[i].url);
          }
        },
  ...
  testWidgets(
    'actual parsed series next and previous follow episode order',
    (tester) async {
      const entries = [
        PlaylistEntry(
          url: 'https://navigation.invalid/e3.mp4',
          title: 'Navigation.S01E03.mkv',
        ),
        PlaylistEntry(
          url: 'https://navigation.invalid/e1.mp4',
          title: 'Navigation.S01E01.mkv',
        ),
        PlaylistEntry(
          url: 'https://navigation.invalid/e2.mp4',
          title: 'Navigation.S01E02.mkv',
        ),
      ];
      await runCase(
        tester,
        entries: entries,
        mode: PlaylistViewMode.series,
        start: 1,
        series: true,
        exercise: (step, menu, current, controls, complete) async {
          for (final i in [2, 0]) {
  ...
  testWidgets(
    'actual shuffle menu exhausts other eligible entries then refills',
    (tester) async {
      final entries = collection();
      final eligible = {
        entries[0].url,
      ...
      await runCase(
        ...
        exercise: (step, menu, current, controls, complete) async {
          ...
          // Even if refill selects the chronological last, EOF keeps shuffling.
          final beforeCompletion = current();
          await complete();
          expect(eligible, contains(current()));
          expect(current(), isNot(beforeCompletion));
          await menu('Continuous shuffle');
