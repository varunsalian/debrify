...
void main() {
  group('xtream stream fetch (real fetch against an in-test panel)', () {
    ...
    bool servesLivePath(String path) {
      ...
      final ids = {
        for (final row in liveRows) row['stream_id'].toString(),
      };
      ...
      server.listen((request) {
        ...
        if (path != '/player_api.php') {
          request.response.statusCode =
              servesLivePath(path) ? HttpStatus.ok : HttpStatus.notFound;
    ...
    test('a category failure degrades to ungrouped channels with a warning',
        () async {
      categoriesFail = true;
      liveRows = [
    ...
    test('VOD streams become /movie/ URLs honoring container_extension',
        () async {
      vodRows = [
        {
          'name': 'Dune',
      ...
      expect(result.channels.last.url, '$base/movie/u4/p4/6.mp4',
          reason: 'mp4 is the documented default');
    });

    test('series become sentinel-URL entries carrying their metadata',
        () async {
      seriesRows = [
        {
          'name': 'Planet Earth',
      ...
      expect(series.attributes['backdrop'], 'http://art/back.jpg',
    ...
    test('series rows without a series_id are dropped', () async {
      ...
      final result = await XtreamCodesService.instance
          .fetchSeriesStreams(base, 'u16', 'p16');
    ...
    test('raw TS is preferred over HLS when a panel serves both', () async {
      ...
      final result = await XtreamCodesService.instance
          .fetchLiveStreams(base, 'u12', 'p12');

      expect(result.channels.single.url, endsWith('.ts'),
    ...
    test('a panel that 404s every dialect freezes no verdict in', () async {
      ...
      final result = await XtreamCodesService.instance
          .fetchLiveStreams(base, 'u13', 'p13');

      expect(result.channels.single.url, '$base/live/u13/p13/15.ts',
          reason: 'falls back to the standard form for this fetch');
      expect(XtreamCodesService.instance.cachedLiveUrlFormCount, 0,
    ...
    test('a detected dialect is remembered for the panel', () async {
      ...
      expect(XtreamCodesService.instance.cachedLiveUrlFormCount, 1,
    ...
    test('a non-numeric stream_id does not send the probe after a phantom '
      ...
      final result = await XtreamCodesService.instance
          .fetchLiveStreams(base, 'u15', 'p15');

      expect(result.channels.last.url, '$base/u15/p15/17.ts',
    ...
    test('a string-typed stream_id still seeds the probe', () async {
      ...
      final result = await XtreamCodesService.instance
          .fetchLiveStreams(base, 'u10', 'p10');

      expect(result.channels.single.url, '$base/u10/p10/13.ts',
    ...
    test('a large payload is built in an isolate and returns intact',
        () async {
      liveRows = [
      ...
      final result = await XtreamCodesService.instance
          .fetchLiveStreams(base, 'u6', 'p6');

      expect(XtreamCodesService.isolateBuilds, 1);
      expect(XtreamCodesService.buildsOnThisIsolate, 0,
    ...
    test('non-ASCII names survive the byte handoff intact', () async {
      ...
      final result =
          await XtreamCodesService.instance.fetchLiveStreams(base, 'u20', 'p20');
    ...
    test('an undeclared charset falls back to latin1, as package:http does',
        () async {
      ...
      streamsRawBytes = utf8.encode(jsonEncode([
        {'name': name, 'stream_id': 7},
      ]));

      final result =
          await XtreamCodesService.instance.fetchLiveStreams(base, 'u21', 'p21');

      expect(result.channels.single.name, latin1.decode(utf8.encode(name)),
    ...
    test('a declared charset is honoured on the worker', () async {
      ...
      final result =
          await XtreamCodesService.instance.fetchLiveStreams(base, 'u22', 'p22');
    ...
    test('a fat first record does not hide the id from the probe', () async {
      ...
      expect(200000, lessThan(XtreamCodesService.streamIdProbeWindow),
          reason: 'the fat record must sit inside the window, not beyond it');

      final result =
          await XtreamCodesService.instance.fetchLiveStreams(base, 'u30', 'p30');

      expect(result.channels.first.url, '$base/u30/p30/1.ts',
    ...
    test('a small payload skips the isolate', () async {
      ...
      expect(XtreamCodesService.isolateBuilds, 0,
          reason: 'spawning an isolate would cost more than the work');
    });

    test('a repeat fetch is served from cache without touching the panel',
        () async {
      liveRows = [
    ...
    test('the cache is keyed by content type, not just the account', () async {
      ...
      final vod =
          await XtreamCodesService.instance.fetchVodStreams(base, 'u18', 'p18');

      expect(vod.channels.single.name, 'A movie',
          reason: 'Movies must never be served the live catalog');
      expect(vod.channels.single.contentType, 'vod');
    });

    test('a non-200 streams response is an error, not an empty catalog',
        () async {
      streamsHttpStatus = HttpStatus.badGateway;

      final result = await XtreamCodesService.instance
          .fetchLiveStreams(base, 'u19', 'p19');
    ...
    test('a panel error object surfaces as an error, not an empty catalog',
        () async {
      streamsError = true;

      final result = await XtreamCodesService.instance
          .fetchLiveStreams(base, 'u7', 'p7');

      expect(result.hasError, isTrue);
      expect(result.error, contains('Account expired'));
      expect(result.channels, isEmpty);
      expect(result.warning, isNull,
          reason: 'the error is the message, not a degradation notice');
    });

    test('a non-JSON body reports the server text rather than crashing', () {
