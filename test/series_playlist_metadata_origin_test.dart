...
// Behavioral pins adapted from f1999322 against the current model, real metadata
// services, current StorageService and the existing player PlaylistEntry type.
// HTTP is controlled; native player and device behavior are outside this suite.
  ...
  await http.runWithClient(() async {
    try {
      ...
      try {
        ...
        expect(
          fixture.consumed,
          fixture.planned.length,
          reason: 'Every explicitly planned request must occur',
        );
        expect(
          fixture.clients.every((client) => client.closed),
          isTrue,
...
SeriesPlaylist _movies() => SeriesPlaylist.fromPlaylistEntries([
  const PlaylistEntry(
    ...
    title: 'Origin.Film.2020.mkv',
  ),
  const PlaylistEntry(
    url: 'https://fixture.invalid/b',
    title: 'Second.Film.2021.mkv',
  ),
], forceSeries: false);
...
void main() {
  ...
  for (final aboveThreshold in [false, true]) {
    test(
      'title-only matching ${aboveThreshold ? 'accepts above' : 'rejects exactly'} 0.6 similarity',
      () async {
        await _withFixture((fixture) async {
          final playlist = _titleOnlyPlaylist('alpha bravo charlie delta echo');
          final middle = playlist.allEpisodes[1];
          final priorInfo = middle.episodeInfo;
          final priorSeriesInfo = middle.seriesInfo;
          final title = aboveThreshold
              ? 'alpha bravo charlie delta golf'
              : 'alpha bravo charlie foxtrot golf';
          fixture.get('$_tv/lookup/shows?imdb=tt4700001', _show(4701));
          fixture.get('$_tv/shows/4701/episodes', [
            _episode(1)..['name'] = 'First Beacon',
            _episode(7)..['name'] = title,
            _episode(9)..['name'] = 'Last Orchard',
          ]);
          await SeriesPlaylistMetadataLoader.fetchEpisodeInfo(
            playlist,
            imdbId: 'tt4700001',
          );
          if (aboveThreshold) {
            expect(middle.episodeInfo!.title, title);
            expect(middle.seriesInfo.season, 1);
            expect(middle.seriesInfo.episode, 7);
          } else {
            expect(middle.episodeInfo, same(priorInfo));
            expect(middle.seriesInfo, same(priorSeriesInfo));
          }
          expect(playlist.allEpisodes[0].episodeInfo!.title, 'First Beacon');
          expect(playlist.allEpisodes[2].episodeInfo!.title, 'Last Orchard');
  ...
  test(
    'saved mapping wins over parameter lookup and mutates existing episodes',
    () async {
      await _withFixture((fixture) async {
  ...
  test(
    'missing parameter lookup falls back to title then bulk episodes',
    () async {
      await _withFixture((fixture) async {
        final playlist = _series();
        fixture.get('$_tv/lookup/shows?imdb=tt2222222', {}, 404);
        fixture.get('$_tv/search/shows?q=origin%20show', [
          {'score': 1, 'show': _show(4102)},
        ]);
        ...
        expect(playlist.allEpisodes.map((e) => e.episodeInfo!.title), [
          'Episode 1',
          'Episode 2',
        ]);
      });
  ...
  test(
    'tt parameter replaces existing field before first suspension',
    () async {
      await _withFixture((fixture) async {
        ...
        fixture.planned.add(
          _PlannedGet('$_tv/lookup/shows?imdb=tt3333333', () {
            requested.complete();
            return response.future;
          }),
        );
        fixture.get('$_tv/shows/4103/episodes', [_episode(1), _episode(2)]);
        final operation = SeriesPlaylistMetadataLoader.fetchEpisodeInfo(
          playlist,
          imdbId: 'tt3333333',
        );
        Object? primary;
        ...
        try {
          expect(
            playlist.imdbId,
            'tt3333333',
          ); // No await before this assertion.
  ...
  test(
    'single episode lookup returns metadata without mutating playlist entries',
    () async {
      await _withFixture((fixture) async {
        final playlist = _series();
        ...
        final result =
            await SeriesPlaylistMetadataLoader.getEpisodeInfoForEpisode(
              'Origin Show',
              1,
              1,
            );
  ...
  test(
    'movie wrapper targets zero and per-index cache survives service reset',
    () async {
      await _withFixture((fixture) async {
        ...
        fixture.get(
          ...
          _movie('Origin Film', '2020', 'tt4444444'),
        );
        ...
        fixture.get(
          ...
          _movie('Second Film', '2021', 'tt5555555'),
        );
        expect(
          await SeriesPlaylistMetadataLoader.fetchMovieMetadataForIndex(
            playlist,
            1,
          ),
          'tt5555555',
        ...
        expect(
          playlist.getImdbIdForIndex(99),
          'tt4444444',
        ); // Getter shared fallback.
      });
    },
  );
  ...
  test('ineligible series and movie branches perform no method HTTP', () async {
    await _withFixture((fixture) async {
      ...
      final noYear = SeriesPlaylist.fromPlaylistEntries([
        const PlaylistEntry(
          url: 'https://fixture.invalid/no-year',
          title: 'Undated.Film.mkv',
        ),
      ], forceSeries: false);
      expect(
        await SeriesPlaylistMetadataLoader.fetchMovieMetadataForIndex(
  ...
  test(
    'numbered episode conversion errors are swallowed by the actual methods',
    () async {
      await _withFixture((fixture) async {
        final playlist = _series();
        final original = playlist.allEpisodes.first;
        ...
        expect(
          playlist.tvmazeShowId,
          4107,
        ); // Earlier mutations are not rolled back.
  ...
  test(
    'movie transport failure returns null without manufacturing metadata',
    () async {
      await _withFixture((fixture) async {
        final playlist = _movies();
        fixture.get('$_cinemeta/manifest.json', {});
  ...
  test(
    'successful index cache precedes bounds and belongs to one playlist',
    () async {
      await _withFixture((fixture) async {
        final first = _movies();
        final other = _movies();
        ...
        MovieMetadataService.clearCache();
        first.allEpisodes
            .removeLast(); // Public mutable list makes index1 invalid.
  ...
  test(
    'distinct indices retain their IDs while first successful completion wins shared',
    () async {
      await _withFixture((fixture) async {
        await _primeMovieAvailability(fixture);
        final playlist = _movies();
        final first = _HeldMovieGet(
          fixture,
          '$_cinemeta/catalog/movie/top/search=origin%20film.json',
          _movie('Origin Film', '2020', 'tt9300001'),
        );
        final second = _HeldMovieGet(
          fixture,
          '$_cinemeta/catalog/movie/top/search=second%20film.json',
          _movie('Second Film', '2021', 'tt9300002'),
        );
        final firstOperation =
            SeriesPlaylistMetadataLoader.fetchMovieMetadataForIndex(
              playlist,
              0,
            );
  ...
  test(
    'same index permits two overlapping requests and last completion overwrites index',
    () async {
      await _withFixture((fixture) async {
        await _primeMovieAvailability(fixture);
        final playlist = _movies();
  ...
  test(
    'unsuccessful movie result is not stored in the model index cache',
    () async {
      await _withFixture((fixture) async {
        final playlist = _movies();
        fixture.get('$_cinemeta/manifest.json', {});
  ...
  test(
    'saved mapping miss enriches through later episode recovery without show state',
    () async {
      await _withFixture((fixture) async {
        final playlist = _series();
  ...
  test(
    'parameter defaults and empty explicit title use four fresh fixtures',
    () async {
      await _withFixture((fixture) async {
        final playlist = _series()..imdbId = 'tt0000000';
        fixture.get('$_tv/search/shows?q=origin%20show', [
        ...
        await SeriesPlaylistMetadataLoader.fetchEpisodeInfo(
          playlist,
        ); // Omitted PARAM, no saved mapping.
  ...
  test(
    'show original-image type error escapes before bulk or show-state updates',
    () async {
      await _withFixture((fixture) async {
        final playlist = _series();
        final first = playlist.allEpisodes.first;
        final firstInfo = first.seriesInfo;
        final response = _show(4301)
          ..['image'] = {'original': 123, 'medium': 'valid-medium'};
  ...
  test(
    'title-only numeric name fails before middle metadata assignment',
    () async {
      await _withFixture((fixture) async {
  ...
  test(
    'title-only string season escapes after middle metadata assignment',
    () async {
      await _withFixture((fixture) async {
  ...
  test(
    'title-only string number escapes after middle metadata assignment',
    () async {
      await _withFixture((fixture) async {
        final playlist = _titleOnlyPlaylist('Middle Harbor');
        final all = playlist.allEpisodes;
...
Future<void> _primeMovieAvailability(_Fixture fixture) async {
  fixture.get('$_cinemeta/manifest.json', {});
  fixture.get(
    '$_cinemeta/catalog/movie/top/search=warmup%20film.json',
...
SeriesPlaylist _titleOnlyPlaylist(String middleName) {
  final episodes = <SeriesEpisode>[
    ...
    SeriesEpisode(
      url: 'https://fixture.invalid/last',
      title: 'Last Orchard',
      filename: 'Last Orchard',
      originalIndex: 2,
