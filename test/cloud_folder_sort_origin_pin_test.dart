...
/// G4-3 origin pin for the folder-view "Sort (A-Z)" ordering that the
...
void main() {
  ...
  Future<void> chooseSortAZ(WidgetTester tester) async {
  ...
  group('TorBox TorboxDownloadsScreen', () {
    ...
    testWidgets('raw view keeps API order; Sort (A-Z) reorders the tree', (
      ...
      await chooseSortAZ(tester);
      expect(renderedOrder(tester, sortedOrder), sortedOrder);

      await drainDeepLinkTimer(tester);
      expect(find.text('Failed to open torrent. Please try again.'), findsNothing);
    });
  });
