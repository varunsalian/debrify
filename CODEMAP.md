...
| `lib/services/torrent_playback_service.dart` | 4 146 |
...
- **`lib/services/torrent_playback/`** — extracted out of that god file (T3).
  ...
  importers are unchanged. All are widget-free. After T3-F retired the ranking /
  search tear-offs, `TorrentPlaybackService` keeps only two `@visibleForTesting`
  tear-offs (`seriesFetcherFor`, `movieFetcherFor`), recorded as closeout debt.
