...
class _VideoPlayerScreenState extends State<VideoPlayerScreen>
  ...
  late final _pikPakRetry = PikPakRetrySession(
    readMounted: () => mounted,
    readStreamDuration: () => _duration,
    readStreamPlaying: () => _isPlaying,
    readDirectDuration: () => _player.state.duration,
    readDirectPlaying: () => _player.state.playing,
    readHeaders: () => widget.httpHeaders,
    openMedia: _openMedia,
    commit: (update) => setState(update),
    showFailureNotice: (skipping) {
      if (skipping) {
  ...
  int _findNextEpisodeIndex() {
    final seriesPlaylist = _seriesPlaylist;
    final sequentialOrder = (seriesPlaylist == null || !seriesPlaylist.isSeries) &&
        (config.viewMode == PlaylistViewMode.raw ||
            config.viewMode == PlaylistViewMode.sorted);
    return _navigation.nextIndex(
      _activePlaylist, seriesPlaylist, _currentIndex, sequentialOrder,
    );
  }
  ...
  int? _pickShuffleIndex() {
    ...
    return _navigation.pickShuffleIndex(
      entries, seriesPlaylist, _currentIndex, widget.viewMode, _random,
    );
  }
  ...
  Future<bool> _playPikPakVideoWithRetry(
    ...
    readCurrentEntry: () =>
        ...
        : null,
    readDiagnosticIndex: () => _currentIndex,
    overrideProvider: overrideProvider,
    overridePikPakFileId: overridePikPakFileId,
    isDebrifyTV: isDebrifyTV,
          ...
          child: ValueListenableBuilder<bool>(
            ...
            child: Stack(
              ...
              children: [
                ...
                if (_pikPakRetry.isRetrying && _pikPakRetry.message != null && !inPip)
