class PlaylistEntry {
  ...
  final String?
  relativePath; // Full relative path from torrent root (e.g. "/Season 1/Episode 1.mkv")
  ...
  final int?
  torboxWebDownloadId; // Torbox web download identifier for lazy resolution
  ...
  final String?
  premiumizeHash; // Premiumize torrent infohash for lazy resolution
  final String? premiumizePath; // Premiumize file path (matched on re-resolve)
  final String?
  premiumizeItemId; // Premiumize cloud item id (lazy resolution from cloud browser)
  final String?
  allDebridLink; // AllDebrid locked link, unlocked on demand (lazy resolution)
  const PlaylistEntry({
    required this.url,
    required this.title,
    this.httpHeaders,
  ...
  PlaylistEntry copyWithTitle(String newTitle) => copyWith(title: newTitle);

  /// Materialize legacy session headers once, before playlist copies or rollback.
  PlaylistEntry withDefaultHttpHeaders(Map<String, String>? defaults) =>
      copyWith(httpHeaders: httpHeaders ?? defaults ?? const {});

  PlaylistEntry copyWith({String? title, Map<String, String>? httpHeaders}) =>
      PlaylistEntry(
        url: url,
