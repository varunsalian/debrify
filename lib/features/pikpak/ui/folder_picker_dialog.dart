...
class _PikPakFolderPickerDialogState extends State<PikPakFolderPickerDialog> {
  final PikPakApiService _apiService = AppServices.pikpak;
  ...
  Future<void> _loadRootFolders() async {
    ...
    try {
      ...
      final folderNodes = folders.map((folder) {
        return _FolderNode(id: folder.id, name: folder.name, level: 0);
      }).toList();
  ...
  Future<void> _createNewFolder(String folderName) async {
    try {
      ...
      final newFolderId = result.id;
      final newFolderName = result.name.isNotEmpty ? result.name : folderName;
