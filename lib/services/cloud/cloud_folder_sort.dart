...
/// Folder-view ordering shared by the cloud files screens.
///
/// Moved verbatim (G4-3) from the Real-Debrid and TorBox files screens, which
/// each carried an identical private copy. "Sort (A-Z)" puts folders first;
/// names that carry a number sort numerically and ahead of the rest, which
/// sort case-insensitively. Pinned by
/// `test/cloud_folder_sort_origin_pin_test.dart`.
class CloudFolderSort {
  ...
  static List<RDFileNode> sortedView(List<RDFileNode> nodes) {
    ...
    folders.sort((a, b) {
      ...
      final aNum = extractSeasonNumber(a.name);
      final bNum = extractSeasonNumber(b.name);
    ...
    files.sort((a, b) {
      ...
      final aNum = extractLeadingNumber(a.name);
      final bNum = extractLeadingNumber(b.name);
  ...
  static int? extractSeasonNumber(String folderName) {
  ...
  static int? extractLeadingNumber(String filename) {
