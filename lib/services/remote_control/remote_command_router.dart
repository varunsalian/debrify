...
class RemoteCommandRouter {
  ...
  // Receiving half of the chunked transfer protocol; owns its own buffers.
  late final RemoteChunkTransferReceiver _chunkTransfers =
  ...
  late final RemoteStaleSourceNotice _staleRemoteNotices =
      RemoteStaleSourceNotice(showNoticeNow: _showSnackBarNow);
  ...
  static const int _maxProfileRemotePayloadBytes =
  ...
  void _scheduleBatchFlush() {
    ...
    _batchIdleTimer = Timer(_batchIdleGap, () {
      ...
      if (_inFlightConfigWork.isNotEmpty ||
          _chunkTransfers.hasActiveTransfers) {
        _scheduleBatchFlush();
        return;
      }
      _flushBatch();
  ...
  Future<void> _dispatchAfterProfileAuthorization(
    ...
    if (!context.encrypted ||
      ...
      _staleRemoteNotices.noteUnauthenticatedDrop(context);
  ...
  void clearProfileSessionState() {
    clearProfileTransferBuffer();
    _chunkTransfers.cancelAll();
  }

  @visibleForTesting
  int get debugStaleRemoteNoticeCount => _staleRemoteNotices.noticeCount;

  @visibleForTesting
  void debugResetStaleRemoteNotices() {
    _staleRemoteNotices.reset();
  }
  ...
  Future<void> _awaitInFlightConfigWork() async {
    ...
    while (_chunkTransfers.hasActiveTransfers &&
        DateTime.now().isBefore(deadline)) {
      await Future.delayed(const Duration(milliseconds: 200));
    }
    if (_chunkTransfers.hasActiveTransfers) {
      debugPrint(
        'RemoteCommandRouter: Giving up on '
        '${_chunkTransfers.activeTransferCount} unfinished '
  ...
  Future<void> _dispatchConfigCommand(
    ...
    switch (command) {
      ...
      case ConfigCommand.debrifyChannelStart:
        _chunkTransfers.handleStart(data, context);
        break;
      case ConfigCommand.debrifyChannelChunk:
        await _chunkTransfers.handleChunk(data, context);
        break;
      default:
