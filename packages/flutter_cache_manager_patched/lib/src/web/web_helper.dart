...
class WebHelper {
  ...
  Stream<FileResponse> _updateFile(String url, String key,
      {Map<String, String>? authHeaders}) {
    return _store.withRevalidation<FileResponse>(key, (existing) async* {
      final cacheObject = existing == null
          ? CacheObject(
              url,
              key: key,
              validTill: clock.now(),
  ...
  Stream<FileResponse> _manageResponse(
    ...
    _store.protectFile(newCacheObject.relativePath);
  ...
  Future<void> _saveFileAndPostUpdates(
    ...
    try {
      ...
      // Failed HTTP streams must not leave a partial UUID file behind.
      try {
        await sink.close();
      } catch (_) {}
      try {
