...
VideoOutput::~VideoOutput() {
  destroyed_ = true;
  // Stop producers before draining the render queue. The atomic flag also
  // prevents callbacks already in flight from scheduling new rendering.
  if (render_context_) {
    mpv_render_context_set_update_callback(render_context_, nullptr, nullptr);
  }

  // Drain any resize already running before choosing which texture to unregister.
  // The platform thread stays free to service Flutter's unregister callback.
  thread_pool_ref_->Post([]() {}).wait();
  if (texture_id_) {
    std::promise<void> unregistered;
    auto future = unregistered.get_future();
    registrar_->texture_registrar()->UnregisterTexture(
        texture_id_, [&unregistered]() { unregistered.set_value(); });
    future.wait();
  }

  // Free libmpv's context on the render worker while its ANGLE context is alive.
  // Waiting here makes the manager's completion a real native teardown barrier.
  auto cleanup = thread_pool_ref_->Post([this]() {
