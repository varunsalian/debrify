...
//
// Operations run in FIFO order on an owned worker, leaving Flutter's platform
// thread available during normal playback. Destruction drains and joins the
// worker before releasing the state used by accepted operations.
class VideoOutputManager {
 public:
  ...
  // Completion runs off the platform thread, after native cleanup finishes.
 ...
 private:
  ...
  // Declare last: join the operation worker before destroying any state it uses.
  // This queue is separate from thread_pool_: output construction/destruction
  // waits for render jobs and must never run on the render worker itself.
