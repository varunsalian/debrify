...
public class SqflitePlugin implements FlutterPlugin, MethodCallHandler {
    ...
    final Map<String, Integer> _singleInstancesByPath = new HashMap<>();
    @SuppressLint("UseSparseArrays")
    final Map<Integer, Database> databaseMap = new HashMap<>();
    ...
    private DatabaseWorkerPool databaseWorkerPool;
    // A handle and its transaction queue belong to one engine, not the process.
    // Includes opens/closes already queued on the worker but not yet completed.
    private final Set<Database> ownedDatabases = new HashSet<>();
    ...
    public void onDetachedFromEngine(FlutterPluginBinding binding) {
        synchronized (databaseMapLocker) {
            ...
            context = null;
            databaseMap.clear();
            _singleInstancesByPath.clear();
            // Queue directly on the owning DB worker, bypassing the abandoned
            // transaction queue. Close rolls back only this dead engine's
            // unfinished transaction on the owning worker before shutdown.
            for (Database database : new ArrayList<>(ownedDatabases)) {
            ...
            databaseWorkerPool.post(
                    ...
                    () -> {
                        ...
                        synchronized (openCloseLocker) {
                            ...
                            synchronized (databaseMapLocker) {
                                // Detach may have happened while SQLite was
                                // opening. Its queued close owns cleanup; never
                                // publish this handle back into the registry.
                                if (!detached) {
                                    if (singleInstance) {
    ...
    // Sqflite.open
    //
    private Runnable onDeleteDatabaseCall(final MethodCall call, final Result result) {
        final String path = call.argument(PARAM_PATH);
        ...
        // The caller runs this only after releasing the admission lock.
        // Workers acquire openCloseLocker before databaseMapLocker, so doing
        // synchronous deletion under databaseMapLocker would invert that order.
    ...
    private void closeDatabase(Database database) {
        synchronized (databaseMapLocker) {
            // An explicit close may have been queued just before detach.
            if (!ownedDatabases.contains(database)) return;
        }
        try {
            database.close();
        } catch (Exception e) {
            Log.e(TAG, "error while closing database " + database.id, e);
        } finally {
            synchronized (databaseMapLocker) {
                ownedDatabases.remove(database);
                stopWorkerIfIdle();
    ...
    private void stopWorkerIfIdle() {
        // The public map excludes pending opens/closes. Do not stop a worker
        // until every admitted handle has finished cleanup.
        if (ownedDatabases.isEmpty() && databaseWorkerPool != null) {
    ...
    public void onMethodCall(MethodCall call, Result result) {
        // Admission and detach are atomic. No old-engine call may queue work
        // behind the close tasks or recreate a worker after detach.
        final Runnable synchronousWork;
        synchronized (databaseMapLocker) {
            if (detached) {
                result.error(Constant.SQLITE_ERROR, "database engine detached", null);
                return;
            }
            synchronousWork = dispatchMethodCall(call, result);
        }
        if (synchronousWork != null) synchronousWork.run();
    }

    private Runnable dispatchMethodCall(MethodCall call, Result result) {
        switch (call.method) {
            // quick testing
            ...
            case METHOD_DELETE_DATABASE: {
                return onDeleteDatabaseCall(call, result);
            }
            case METHOD_DEBUG: {
    ...
    void onOptionsCall(final MethodCall call, final Result result) {
        ...
        if (threadCount != null && !threadCount.equals(THREAD_COUNT)) {
            if (!ownedDatabases.isEmpty()) {
                result.error(Constant.SQLITE_ERROR, "Cannot change worker count with open databases", null);
                return;
            }
