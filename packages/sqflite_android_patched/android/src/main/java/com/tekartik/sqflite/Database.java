...
class Database {
    ...
    public void close() {
        ...
        try {
            for (SqfliteCursor cursor : cursors.values()) {
                try {
                    cursor.cursor.close();
                } catch (Exception e) {
                    Log.e(TAG, "error closing retained cursor", e);
                }
            }
            // Android can retain an acquired SQLiteSession connection after
            // SQLiteDatabase.close(). End the abandoned transaction on its
            // owning worker before closing the pool and quitting that worker.
            // Called only for explicit connection close or engine teardown.
            if (isInTransaction()) sqliteDatabase.execSQL("ROLLBACK");
        } finally {
            transactionDepth = 0;
            currentTransactionId = null;
            cursors.clear();
            noTransactionOperationQueue.clear();
